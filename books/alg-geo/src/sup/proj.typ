...
#proposition(number: "PJ-4", title: "High-degree invariance")[
...
#proposition(number: "PJ-6", title: "Graded sections and high degrees")[
...
#proof[
  ...
  + *Prove eventual exactness of the localization complex.* Here is the precise graded-algebra computation needed for its eventual
    ...
    degrees, also for each of the finitely many shifts. Check this Laurent-
    monomial computation of the complex (Check!). The long exact
  ...
  + *Characterize the zero associated sheaf.* For the zero-sheaf assertion, triviality of $shf.o (1)$ on $U_i$ implies
    ...
    so all sufficiently high degree pieces vanish. The monomial bound is
    an algebraic check (Check!). Conversely, eventual
...
#proof[
  ...
  + *Identify the rational normal curve.* For $k[x_0,x_1]$, the Veronese ring is generated in degree one by
    ...
    ring on these generators gives a closed immersion by PJ-5.
...
#metadata(none) <sup-pj-11>
#corollary(number: "PJ-11", title: "Very ampleness and composition")[
...
#proof[
  ...
  + *Realize the tensor product by Segre.* Let $i:X->sch.p^r_Y$ and $j:X->sch.p^s_Y$ realize the two sheaves.
    ...
    by MG-5, and base change preserves immersions. Compose with the Segre
    closed immersion. Its pullback of $shf.o (1)$ is the tensor product by
    PJ-10, proving the first assertion. If $i$ is closed, both factors are
    closed immersions.
...
#metadata(none) <sup-pj-12>
#proposition(number: "PJ-12", title: "Projective normality")[
...
#proof[
  ...
  + *Identify the normal affine coordinate rings.* The homogeneous ideal of an integral closed subscheme is prime, so its
    ...
    Put $A_i=(S_(x_i))_0$. Normality of $X$, and XP-12, say that each
  ...
  + *Describe the section ring as an intersection.* Sheaf gluing on these charts identifies
    ...
    (Check! polynomial rings over integrally closed domains are integrally closed).
    Hence their intersection is integrally closed and contains every element
    of $K$ integral over $S$.

  + *Identify the integral closure.* PJ-6 applied to $M=S$ identifies $S_n$ with $S'_n$ for $n>=N$.
    ...
    A module-finite algebra is integral (Check!), so $S'$ is exactly the integral
    closure of $S$ in $K$.
  ...
  + *Pass to a high Veronese subring.* Also $S'_0=k$: it is a finite dimensional domain over $k$, hence a
    ...
    modulo $d$. PJ-9 now proves projective normality of this embedding.
  ...
  + *Prove the projective-normality criterion.* Finally, if $S$ is normal, then each $(S_(x_i))_0$ is normal (compare
    ...
    ideal. This proves the equivalence. XP-12 supplies the last reduction
