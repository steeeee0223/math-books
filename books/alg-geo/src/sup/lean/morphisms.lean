...
notions without the hypothesis in MG-9. `LocallyQuasiFinite` alone is not
...
/-- MG-8: finiteness gives the complete properness assertion. -/
theorem finite_is_proper {X Y : Scheme} (f : X ⟶ Y) [IsFinite f] : IsProper f := by
  infer_instance

/-- MG-10's terminal descent, after constructing the proper surjection onto the image. -/
...
/-- MG-6: the map B/I → B/J for I ≤ J is surjective. -/
