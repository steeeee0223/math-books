...
#definition(title: "Abelian Category")[
  An _abelian category_ is a category $Af$ such that: for any objects $A,B in Ob Af$,
...
#definition(title: "Cochain Complex & Cohomology")[
  ...
  - The _$i$th cohomology object_ $h^i(A^op("bullet"))$ of the complex $(A^op("bullet"),d)$ is defined to be $Ker d^i/ Im d^(i-1)$.
...
#remark(title: "Covariant & Contravariant Functors")[
  - *(Additive Functor).* A covariant [resp. contravariant] functor $F: Af to Bf$ between two abelian categories is _additive_ if for any objects $A,A' in Ob Af$, the induced map $op("Hom")_Af (A,A') to op("Hom")_Bf (op("FA"),op("FA")')$ [resp. $op("Hom")_Af (A,A') to op("Hom")_Bf (op("FA")',op("FA"))$] is a homomorphism of abelian groups.
...
#definition(title: "Resolutions")[
  A $( ast )$-_resolution_ of an object $A in Ob Af$ is a complex $C^op("bullet")$ together with a morphism $op("epsilon"):A to C^0$ such that every $C^i in Ob Af$ has property $( ast )$, and that the sequence
  $
...
#definition(title: "Injective Objects")[
  - An object $I in Ob Af$ is _injective_ if the functor $op("Hom")_Af (-,I)$ is exact.
...
#proof[
  ...
  - For $i=1$, we shall find an $h^1:I^1 to J^0$ satisfying $h^1d_I^0=f^0-g^0$. First, since $d_I^0op("epsilon")_A=0$, we may factor  $d_I^0:I^0attach(arrow.r.twohead, t: "p^0") Coker op("epsilon")_op("Aattach")(arrow.r.tail, t: "i^0")I^1$. Further, note that $(f^0-g^0)op("epsilon")_A=op("epsilon")_B(f-f)=0$, factor $f^0-g^0:I^0attach(arrow.r.twohead, t: "p^0") Coker op("epsilon")_op("Aattach")(arrow.r.tail, t: "j^0")J^0$. Apply the exact functor $op("Hom")_Af (-,J^0)$ to $0 to Coker op("epsilon")_op("Aattach")(arrow.r.tail, t: "i^0")I^0$, we have $op("Hom")_Af (I^1,J^0)attach(arrow.r.twohead, t: "- compose i^0")op("Hom")_Af ( Coker op("epsilon")_A,J^0) to 0$. So, $exists \ h^1 in op("Hom")_Af (I^1,J^0) mapsto j^0$. Finally,
  $
    f^0-g^0=j^0p^0=h^1i^0p^0=h^1d_I^0.
  $
  - Suppose the $n$th step holds, i.e. we have such $h^i$ for all $i <= n$ such that $f^i-g^i=h^(i+1)d_I^i+d_J^(i-1)h^i$ for all $i <= n-1$. First, factor $d_I^n:I^op("nattach")(arrow.r.twohead, t: "p^n") Coker d_I^(n-1)attach(arrow.r.tail, t: "i^n")I^(n+1)$ as $d_I^op("nd")_I^(n-1)=0$. Note that
  $
  ...
  $
  Factor $f^n-g^n-d_J^(n-1)h^n:I^op("nattach")(arrow.r.twohead, t: "p^n") Coker d_I^(n-1)attach(arrow.r.tail, t: "j^n")J^n$. Apply the exact functor $op("Hom")_Af (-,J^n)$ to $0 to Coker d_I^(n-1)attach(arrow.r.tail, t: "i^n")I^(n+1)$, we have $op("Hom")_Af (I^(n+1),J^n)attach(arrow.r.twohead, t: "- compose i^n")op("Hom")_Af ( Coker d_I^(n-1),J^n) to 0$. So, $exists \ h^(n+1) in op("Hom")_Af (I^(n+1),J^n) mapsto j^n$. Finally,
  $
    f^n-g^n-d_J^(n-1)h^n=j^op("np")^n=h^(n+1)i^op("np")^n=h^(n+1)d_I^n.
  $
]
#definition(title: "Derived Functor")[
  Let $Af$ be an abelian category with enough injectives, and let $F: Af to Bf$ be a covariant left exact functor. We construct the _right derived functors_ $R^op("iF")_i >= 0$ of $F$ as follows. For each $A in Ob Af$, choose once and for all an injective resolution $I^op("bullet")$ of $A$. Then we define $R^op("iF")(A) := h^i(F(I^op("bullet")))$.
]
