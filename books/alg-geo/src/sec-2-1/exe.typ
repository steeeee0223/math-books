...
#exercise(title: "Exercise 1.8")[
  ...
  exact; compare Exercise 1.21.
]

// Hartshorne II.1, Exercise 1.9
#exercise(title: "Exercise 1.9 (Direct Sum)")[
  For sheaves $shf.f_i$ on $X$, show that $U mapsto bigOPlus(shf.f_i (U), i)$ is a
  sheaf. Show that the resulting $bigOPlus(shf.f_i, i)$ is both direct sum and
  direct product in the category of sheaves of abelian groups.
]

// Hartshorne II.1, Exercise 1.10
#exercise(title: "Exercise 1.10 (Direct Limit)")[
  For a direct system of sheaves $shf.f_i$, define $op("colim") shf.f_i$ as the sheaf
  associated to $U mapsto op("colim") shf.f_i (U)$. Prove its universal property as a
...
#exercise(title: "Exercise 1.11")[
  ...
  $U mapsto op("colim") shf.f_i (U)$ already is a sheaf. In particular,
  $Gamma(X, op("colim") shf.f_i)=op("colim") Gamma(X, shf.f_i)$.
]
...
#proof[
  Let $shf.f'$ be the presheaf
  $U mapsto op("colim")_(V supset.eq f(U)) (f_*shf.f)(V)
  =op("colim")_(f^(-1) (V) supset.eq U) shf.f (f^(-1) (V))$.
  Restriction to $U$ gives $shf.f'->shf.f$, and sheafification gives the counit
  $f^(-1)f_*shf.f->shf.f$.

  Conversely, $V supset.eq f(f^(-1) (V))$ gives
  $shf.g (V)->(f^(-1)shf.g)'(f^(-1) (V))->(f^(-1)shf.g)(f^(-1) (V))$; these maps form
  the unit $shf.g->f_*f^(-1)shf.g$.

  Sending $alpha:f^(-1)shf.g->shf.f$ to the composite
  $shf.g->f_*f^(-1)shf.g->^(f_*alpha)f_*shf.f$, and sending
  $beta:shf.g->f_*shf.f$ to
  $f^(-1)shf.g->^(f^(-1)beta)f^(-1)f_*shf.f->shf.f$, defines inverse natural maps.
]
